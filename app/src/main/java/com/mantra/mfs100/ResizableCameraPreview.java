...
public class ResizableCameraPreview extends CameraPreview {
     ...
     */
    public ResizableCameraPreview(Activity activity, LayoutMode mode, boolean addReversedSizes,int w,int h,Camera camera) {
        super(activity,  mode,camera);
        super.w=w;
        super.h=h;
        if (addReversedSizes) {
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        mCamera.stopPreview();
        Log.d("aj", "this is surfacechanged ResizableCamera");
        Camera.Parameters cameraParams = mCamera.getParameters();
        boolean portrait = isPortrait();

        if (!mSurfaceConfiguring) {
        	Log.d("aj", "this is !mSurfacechanged in surfacechanged ResizableCamera");
        	
            Camera.Size previewSize = determinePreviewSize(portrait, width,300);
    ...
    public void setPreviewSize(int index, int width, int height) {
        mCamera.stopPreview();
        Log.d("aj", "this is setPreview size ResizableCamera");
        Camera.Parameters cameraParams = mCamera.getParameters();
        boolean portrait = isPortrait();
