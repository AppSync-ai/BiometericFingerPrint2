...
public class GetCityForcastByZipCodeTask extends
		AbstractProgressableAsyncTask<SoapObject,ArrayList<CityForecastBO> >{

	private static final String TAG = "GetStudentDetails";
	...
	private static final String WS_NAMESPACE = "http://tempuri.org/";
	private static final String WS_METHOD_NAME = "GetStudentDetails";
	String IMEI,VTP,BATCHID;
	public GetCityForcastByZipCodeTask(String imei,String vtp,String batch) {
		this.IMEI =imei;
		this.VTP =vtp;
		this.BATCHID =batch;
	}
	...
	@Override
	protected ArrayList<CityForecastBO> performTaskInBackground(SoapObject parameter)
			throws Exception {
		...
		//
		ArrayList<CityForecastBO> result = null;
	...
	private ArrayList<CityForecastBO> parseSOAPResponse(SoapObject response) {
		ArrayList<CityForecastBO> cityForecastResult = null;
		cityForecastResult=new ArrayList<CityForecastBO>();
		SoapObject cityForecastNode = (SoapObject) response
				.getProperty("GetStudentDetailsResult");
...
//			boolean isSuccess = Boolean.parseBoolean(cityForecastNode
//					.getPrimitivePropertySafelyAsString("Success"));
...
//			 name="Forecast"> element in wsdl
			 ...
			 cityForecastResult.add(new CityForecastBO(date, weatherId));
			// new MyDataBase(context).insertStudentDetails(batchid, vtpno, name, id);
